...
def main():
	...
	#边训练边测试        
        ...
        if epoch%10==0:
            torch.save(model.state_dict(), '/home/xlc/zjr/Resnet/data/model/ResNet_cub200'+'/params_3_' + str(epoch) + '.pkl')
        if is_acc_change:
            torch.save(model.state_dict(), '/home/xlc/zjr/Resnet/data/model/model_resnet152_1/model13' + '/res152_cub200_params_acc=' + str(log_acc[len(log_acc) - 1]) + '_epoch=' + str(epoch) + '.pkl')
        # adjust_learning_rate(optimizer, epoch, args)
        # if epoch%10==0:
